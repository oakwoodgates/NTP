...
|-----------|-----------|-------|
| Trading engine | NautilusTrader 1.227.0 | Pinned version, pip dependency |
| Persistence | asyncpg → PostgreSQL 16 + TimescaleDB | Actors write via asyncpg in executor threads |
| Migrations | Alembic | |
| Cache | Redis (auth required) | NT native cache (positions, orders, account state). `REDIS_PASSWORD` MUST be set in `.env`; container is launched with `--requirepass` and the trader fails at `node.build()` with `NOAUTH` if missing. Bound to `127.0.0.1` only on the host. |
...
- **Every table includes `run_id`** (UUID) for grouping rows by TradingNode run. `strategy_runs` additionally self-FKs via `parent_run_id` (PR #39) linking consecutive runs of the same `(trader_id, instrument_id, strategy_id, run_mode)` tuple; walk the chain with a recursive CTE for cross-restart analyses (see `docs/PAPER_TRADING_GUIDE.md` "Cross-restart queries").
...
├── adapters/         # Project-side NT-adapter patches (must stay tightly scoped)
│   └── patched_sandbox.py  # SandboxExecutionClient + BestPriceFillModel
│                           # (NT 1.227.0 partial-fill workaround;
│                           # see docs/SANDBOX_PARTIAL_FILL_AUDIT.md)
...
**Phase 2.5 — Paper-trading revalidation.** Stage A live on Hyperliquid sandbox: NT 1.227 + PR #42 warm-restart + PR #45 hardening + PR #46 pgdata mount + PR #47 shared image + PR #48 no-flatten-on-stop, all on the canonical sizing knobs from the Phase 2.5 plan. Stage B (HL testnet, 2 weeks) is gated on Stage A's wiring-pass criteria. See [`docs/ROADMAP.md`](docs/ROADMAP.md), [`docs/PAPER_TRADING_GUIDE.md`](docs/PAPER_TRADING_GUIDE.md), [`docs/DEPLOY.md`](docs/DEPLOY.md). Specific deployed config + run_id stay on the DO box per the "findings stay local" rule.
...
1. Build trader image: `docker compose build trader` (single `ntp-trader:latest` image shared by every profile-gated trader service — `trader`, `trader-eth`, `trader-btc`, `trader-sol` — per PR #47)
2. Start infra: `docker compose up -d` (no trader yet — infra only). Requires `REDIS_PASSWORD` in `.env` (Redis container launched with `--requirepass`).
...
**Graceful shutdown:** `docker compose stop <svc>` sends SIGTERM → Python SIGTERM handler raises `KeyboardInterrupt` → `finally` block calls `node.stop()`, `node.dispose()`, `_close_run()`. Verify `strategy_runs.stopped_at` is not NULL after stop. Per PR #48, the runners construct every strategy config with `close_positions_on_stop=False` so this shutdown does NOT flatten open positions — orders are cancelled, positions stay open in NT's Redis cache, PR #42 persists strategy-private state, and the next start rehydrates both. Code deploys are position-neutral.
...
- **Actor has no `create_task`.** NT 1.227.0 Actor uses `run_in_executor` (ThreadPoolExecutor) for I/O, not `create_task`. Strategy has different APIs than Actor.
...
- **HyperliquidExecClientConfig uses `private_key` + `vault_address`** (NOT `wallet_address`). Verified in NT 1.227.0. The `testnet=` kwarg was removed in 1.226 — use `environment=HyperliquidEnvironment.{TESTNET,MAINNET}` (re-exported pyo3 enum, requires `# type: ignore[attr-defined]` on the import).
...
- **Margin is not enforced on MARGIN accounts in NT 1.227.0.** RiskEngine short-circuits the margin check (`risk/engine.pyx:678`); no liquidation engine exists in the backtester (verified by exhaustive grep, unchanged from 1.225 and re-verified at the 1.226 → 1.227 upgrade). The project simulates this via a `LiquidationAware` strategy mixin + `AccountAliveMonitor` actor. Opt in by passing `LiquidationConfig(enabled=True)` and `venue_config=` to `make_engine`; pass the resolved config into the strategy too. **Inheritance order is non-negotiable**: `class Foo(LiquidationAware, Strategy):` (mixin first; reverse silently disables it). **Disable in live** (`run_live.py`) — venues handle their own liquidation; currently no auto-detection. See `docs/LIQUIDATION_AND_SIZING.md`.
...
- **MACross persists cross-gate + mixin state across container restarts** (PR #42). `MACross.on_save`/`on_load` write `cross_gate_last_signal`, `cross_gate_bootstrap_pending`, `liq_order_ids`, `liq_count`, `protective_order_ids`, `protective_count` into NT's Redis cache. On restart, NT rehydrates orders/positions from the cache and the strategy rehydrates its private state — no spurious post-restart entries, no double-stops, no synthetic re-bootstrap. Requires `save_state=True` + `load_state=True` on `TradingNodeConfig` (already set in `run_sandbox.py` / `run_live.py`). See `docs/STRATEGY_ENTRY_RULES.md` "Cross-gate state persists across container restarts" + `docs/PROTECTIVE_STOP_RESTART_AUDIT.md`.
- **Paper/live runners override `close_positions_on_stop=False`** (PR #48). `MACrossConfig.close_positions_on_stop` defaults to `True` for backtests (analyzer wants flat positions at end of run). `run_sandbox.py` and `run_live.py` build every strategy config with `False` so `docker compose stop`/`restart`/`up -d --force-recreate` does NOT flatten open positions — code deploys are position-neutral, PR #42 picks state back up on next start. Without the override every deploy generates a synthetic `shutdown_flatten` exit that pollutes the trade history. See `docs/PAPER_TRADING_GUIDE.md` "Deploy lifecycle — graceful stop is position-neutral".
- **Single shared `ntp-trader:latest` image across all profile-gated trader services** (PR #47). The `x-trader-common` YAML anchor declares `image: ntp-trader:latest` so `trader`, `trader-eth`, `trader-btc`, `trader-sol` all share one image identity. A single `docker compose build trader` updates every profile. Without the explicit `image:` pin, compose generates one image per service from the same `build:` context and `docker compose build trader` silently leaves the per-instrument images stale — bit us 2026-05-22 when post-PR45 `trader-eth` crashloop'd on `NOAUTH` for 18 minutes because its image hadn't been rebuilt.
- **LiquidationAware terminates the cooperative `super().on_start()` chain** (PR #44). `LiquidationAware.on_start` is intentionally a no-op that does NOT call `super().on_start()`. This stops the cooperative-super chain before reaching NT's base `Strategy.on_start` stub, which logs a misleading `"handler was called when not overridden"` warning on every start. New mixins that compose with `LiquidationAware` must either be placed BEFORE it in the MRO (their `super().on_start()` then resolves to `LiquidationAware.on_start` which terminates), or follow the same no-super-on-start pattern. Tests at `tests/unit/test_liquidation_mixin.py::TestOnStartTerminator`.
- **Postgres `pgdata` volume mount target must match the image's `PGDATA` env var** (PR #46). For `timescale/timescaledb:2.x-pg16` that's `/var/lib/postgresql/data`. If the mount target doesn't match, Postgres writes to the container's writable layer instead of the volume — every `docker compose down` silently destroys the database. Latent in this repo from 2026-05-15 → 2026-05-22 (stale `/home/postgres/pgdata/data` from an older timescale image convention); cost six days of paper-trade history when the hardening PR's full down/up cycle finally tripped it. Regression test at `tests/unit/test_compose_pgdata_mount.py` pins the invariant; re-verify after every timescale image bump (`docker run --rm timescale/timescaledb:<ver> env | grep PGDATA`).
- **`SandboxExecutionClient` partial-fills MARKET orders larger than `bar.volume / 4`** (NT 1.227.0 bug). NT's matching engine builds an L1 book from live bars decomposed into 4 synthetic ticks; market orders that exceed the per-tick size get a partial fill, and the "slip-fill the residual" safety net at `backtest/engine.pyx:7299-7336` silently no-ops because `LiveExecutionEngine.process()` enqueues fill events asynchronously and the order's FSM hasn't transitioned from `SUBMITTED` to `PARTIALLY_FILLED` when the gate is checked. Result: zombie order in `orders_open` forever, no completion event, no future iteration. Cost us -16.66% of a paper account in 6 days. **Workaround applied:** `src/adapters/patched_sandbox.py::PatchedSandboxExecutionClient` swaps in NT's built-in `BestPriceFillModel` (1M units at best bid/ask → MARKET orders fill in one event, slip-fill code path never runs). Registered via `PatchedSandboxLiveExecClientFactory` in `scripts/run_sandbox.py`. Bug reproduces in `tests/integration/test_sandbox_partial_fill.py`; patch verified in `tests/unit/test_patched_sandbox.py`. Re-verify after every NT upgrade per `docs/SANDBOX_PARTIAL_FILL_AUDIT.md` §7 — if the default-FillModel test starts failing, upstream NT fixed the race and the workaround can be removed.
